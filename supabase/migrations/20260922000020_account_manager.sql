-- Account Manager (plan 22-09, C4): runs the business for the Owner. The client signs it in as
-- store role "owner", so every entry it appends carries role "owner", and like the Owner it may
-- change config and add lots. Sales money is hidden on screen only: app_state is one payload that
