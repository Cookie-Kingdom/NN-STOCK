...
async function reachPreSmoke(page: Page) {
  ...
  await step(
    page,
    "Chef House: ยืนยันรับเนื้อ 500 กก. → stage ก่อนสโมค",
    async () => {
      await signInAs(page, ACCOUNTS.chef);
...
test("D1–D11 Chef House ผลิต → กลับสต๊อกกลาง: รับ 500 → ก่อนสโมค 480 → สโมค 2 รอบ 476 → Edit 475 → ปิด Lot → ขากลับ 475 → Foodiva 474 → สต๊อกกลาง 474", async ({
  ...
  await step(
    ...
    async () => {
      ...
      await expect(dialog(page)).toContainText(
        "ส่งกลับกรุงเทพฯ 2 ถุง · น้ำหนักรวม 238.00 กก.",
      );
      await saveEntry(page);
  ...
  await step(
    page,
    "Chef House: D4 รอบ 2 เข้าเตา 240 ถุง 80 + 80 + 78 waste 2 → 5 ถุง 476 กก. stage ปิด Lot",
    async () => {
      await button(page, "บันทึก Lot สโมครายวัน");
      ...
      await expect(
        rowIn(page, SMOKE_LOG, "3 ถุง · 80.00 × 2 + 78.00 × 1 กก.")
          .getByRole("cell")
          .nth(7),
      ).toHaveText("0.00 กก.");
  ...
  await step(
    ...
    async () => {
      ...
      await submitAndExpectError(
        page,
        "กรอกน้ำหนักถุงใหญ่ให้ครบและมากกว่า 0 ทุกรอบ",
      );
  ...
  await step(
    page,
    "Chef House: D5 Edit รอบ 2 waste 3 ถุง 80 + 80 + 77 → หลังรมรวม 475 กก. 5 ถุง",
    async () => {
      const edit = dialog(page);
  ...
  await step(
    page,
    "Chef House: D6 ยืนยันปิด Lot → stage ขนส่ง Chef House → Foodiva · ปุ่ม Edit หาย · count pill งานผลิตหมด",
    async () => {
      await signInAs(page, ACCOUNTS.chef);
      await tab(page, "งานผลิต");
  ...
  await step(
    ...
    async () => {
      await signInAs(page, ACCOUNTS.foodiva);
      await expect(
        tableSection(page, "เนื้อรมควันรอ Foodiva รับเข้าตู้").getByRole(
          "button",
          {
  ...
  await step(
    ...
    async () => {
      ...
      await field(page, /น้ำหนักส่งจาก Chef House/, "475");
      await saveEntry(page);
      const row = tableSection(page, "รายการขนส่งตาม Lot")
        .locator("tbody tr")
        .first();
  ...
  await step(
    ...
    async () => {
      ...
      const waiting = rowIn(
        page,
        "เนื้อรมควันรอ Foodiva รับเข้าตู้",
        "กท 1002",
      );
  ...
  await step(
    page,
    "Owner: D9 รับเข้าสต๊อกกลาง 300 → ต้องกรอกเหตุผล · 474 → สต๊อกกลาง 474 กก. 5 ถุง stage จัดสรร / ขาย",
    async () => {
      await signInAs(page, ACCOUNTS.owner);
      await tab(page, "รับเนื้อเข้าสต๊อกกลาง");
  ...
  await step(
    page,
    "Owner: D9 ถุงทุกใบถูก pro-rate ตามสต๊อกกลาง 474 / 475 และทุกใบ > 0",
    async () => {
      ...
      await expect(
        allocation.getByRole("combobox", { name: /^เลือกสาขาให้ถุงที่/ }),
      ).toHaveCount(5);
      ...
      const central = page
        ...
        .filter({ hasText: "ถุง พร้อมจัดสรร" });
  ...
  await step(
    ...
    async () => {
      ...
      await expect(
        smokeRows.filter({
          hasText:
            "เข้าเตา 240.00 กก. · หลังรม 237.00 กก. · Waste 3.00 กก. · 3 ถุง",
        }),
      ).toHaveCount(1);
      await expect(
        smokeRows.filter({
          hasText:
            "เข้าเตา 240.00 กก. · หลังรม 238.00 กก. · Waste 2.00 กก. · 2 ถุง",
        }),
      ).toHaveCount(1);
...
test("E2E-D1: Edit ข้อมูลก่อนปิด Lot น้ำหนักรับจริง 0 ต้องขึ้นข้อความ กรอกน้ำหนักให้ถูกต้อง ในฟอร์ม ไม่ใช่ bubble ของเบราว์เซอร์ (D5)", async ({
  ...
  await step(
    page,
    "Chef House: Edit น้ำหนักรับจริง 0 → ข้อความในฟอร์ม",
    async () => {
      await button(page, "Edit ข้อมูลก่อนปิด Lot");
