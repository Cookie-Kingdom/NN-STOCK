...
async function setMaterialPars(
  page: Page,
  par: (i: number) => string,
...
test("Lane B: B1–B2 Owner ตั้งค่าครบทุก section · validation ไม่บันทึก · ออก/เข้าใหม่ค่ายังอยู่", async ({
  ...
  await step(
    page,
    "Owner: B2 ขณะแก้ section หนึ่ง section อื่นล็อก (กำลังแก้ตารางอื่น)",
    async () => {
      const pricing = tableSection(page, S_PRICING);
      await pointAndClick(
        page,
      ...
      await expect(
        tableSection(page, S_BRANCH).getByRole("button", {
          name: "กำลังแก้ตารางอื่น",
        }),
      ).toBeDisabled();
      await pointAndClick(
        page,
        pricing.getByRole("button", { name: CANCEL_EDIT }),
      );
      await expect(
        tableSection(page, S_BRANCH).getByRole("button", {
          name: REQUEST_EDIT,
  ...
  await step(
    page,
    "Owner: B2 ราคากล่อง -5 / ว่าง / ตัวอักษร → เป็นตัวเลขตั้งแต่ศูนย์ ไม่บันทึก · packKg 0 → มากกว่าศูนย์",
    async () => {
  ...
  await step(
    page,
    "Owner: B2 ค่าขนส่งขาไป -1 และ tolerance 101 → บล็อก",
    async () => {
      await sectionRefuses(
        page,
        S_PRODUCTION,
  ...
  await step(
    page,
    "Owner: B2 จำนวนฐานวัสดุ -1 → จำนวนฐาน กล่องพิมพ์ลาย ตั้งแต่ศูนย์ · ตารางวัสดุยังเป็น 0",
    async () => {
      await sectionRefuses(
        page,
        S_MATERIALS,
      ...
      await expect(
        cellOf(tableSection(page, S_MATERIALS), [BOX], 1),
      ).toHaveText("0.00 ชิ้น");
    },
  );
  ...
  await step(
    page,
    "Owner: B1 ข้อมูลใบ PO (โลโก้ Foodiva Chef House)",
    async () => {
      await editSection(page, S_DOCS, async () => {
        await page
          .getByLabel("อัปโหลดโลโก้ NerdNuea")
          .setInputFiles(
            path.join(process.cwd(), "tests/fixtures/logo-demo.png"),
          );
  ...
  await step(
    page,
    "Owner: B1 ราคาขาย 370/340/0.101/35 · วัตถุดิบสาขา · ค่าขนส่ง 1500/1400/2600",
    async () => {
  ...
  const expectSettings = async () => {
    ...
    await expect(docs.getByAltText("โลโก้ NerdNuea")).toHaveAttribute(
      "src",
    ...
    const pricing = tableSection(page, S_PRICING);
    await expect(cellOf(pricing, ["ราคากล่องมาตรฐาน"], 1)).toHaveText(
      "฿370.00",
    );
    await expect(cellOf(pricing, ["ราคาเนื้อซีลเพิ่ม"], 1)).toHaveText(
      "฿340.00",
    );
    await expect(cellOf(pricing, ["น้ำหนักเฉลี่ยต่อซีล"], 1)).toHaveText(
      "101.00 กรัม",
    );
    await expect(cellOf(pricing, ["ราคาขายน้ำพริกหลอด"], 1)).toHaveText(
      "฿35.00",
    );
    const supplies = tableSection(page, S_SUPPLIES);
    await expect(cellOf(supplies, ["จำนวนฐานข้าวเหนียวดิบ"], 1)).toHaveText(
      "25.00",
    );
    await expect(cellOf(supplies, ["ราคาต่อหน่วยข้าวเหนียวดิบ"], 1)).toHaveText(
      "฿55.00",
    );
    await expect(cellOf(supplies, ["จำนวนฐานน้ำพริก"], 1)).toHaveText("120.00");
    await expect(cellOf(supplies, ["ราคาต่อหน่วยน้ำพริก"], 1)).toHaveText(
      "฿22.00",
    );
    await expect(cellOf(supplies, ["จำนวนฐานข้าวเหนียวสุก"], 1)).toHaveText(
      "35.00",
    );
    await expect(cellOf(supplies, ["ราคาต่อหน่วยข้าวเหนียวสุก"], 1)).toHaveText(
      "฿48.00",
    );
    const production = tableSection(page, S_PRODUCTION);
    await expect(cellOf(production, ["ค่าขนส่งขาไป"], 1)).toHaveText(
      "฿1,500.00",
    );
    await expect(cellOf(production, ["ค่าขนส่งขากลับ"], 1)).toHaveText(
      "฿1,400.00",
    );
    await expect(cellOf(production, ["ค่าขนส่งไป-กลับ"], 1)).toHaveText(
      "฿2,600.00",
    );
    ...
    for (let i = 0; i < MATERIALS.length; i += 1) {
      ...
      await expect(cellOf(materialTable, [MATERIALS[i]], 2)).toHaveText(
        `฿${3 + i}.00 / ชิ้น`,
      );
    }
  };
  ...
  await step(
    page,
    "Owner: B1 ทุก section แสดงค่าที่บันทึก · แบนเนอร์ตั้งค่าวัสดุยังไม่ครบหายไป",
    async () => {
  ...
  await step(
    page,
    "ระบบ: B1 reload + ออกจากระบบแล้วเข้าใหม่ → ค่ายังอยู่ทุกช่อง",
    async () => {
      await page.reload();
  ...
  await step(
    page,
    "Owner: B1 ขอแก้ไขอีกครั้ง → ช่องกรอกโหลดค่าที่บันทึก (ไม่ใช่ค่า seed) แล้วยกเลิก",
    async () => {
      const pricing = tableSection(page, S_PRICING);
      await pointAndClick(
        page,
  ...
  await step(
    page,
    "Owner: B1 ฐาน/ราคาวัสดุใช้กับทั้ง 2 สาขาในสต๊อกของทั้งหมด",
    async () => {
...
test("Lane B: B3–B6, B8 ซื้อวัสดุ → ส่งสาขา → สาขายืนยันรับ · น้ำพริก · ค่าใช้จ่าย Owner · ยกเลิกรายการจาก Log", async ({
  ...
  await step(
    page,
    "Owner: B3 ซื้อวัสดุ — ไม่ติ๊ก → ติ๊กเลือกอย่างน้อย 1 รายการ",
    async () => {
      await signInAs(page, ACCOUNTS.owner);
      await tab(page, "สต๊อกของทั้งหมด");
  ...
  await step(
    page,
    "Owner: B3 ไม่กรอกผู้จำหน่าย → กรอกผู้จำหน่าย (inline) · จำนวน 0 / 2.5 → จำนวนเต็มที่มากกว่า 0",
    async () => {
      ...
      await submitAndExpectError(
        page,
        `กรอกจำนวน ${BOX} เป็นจำนวนเต็มที่มากกว่า 0`,
      );
  ...
  await step(
    page,
    "Owner: B3 ซื้อครบ 7 รายการ 100…160 ชิ้น @ ฿2 → ยอดรวม ฿1,820",
    async () => {
      await field(page, `จำนวนซื้อ ${BOX}`, "100");
      ...
      await expect(dialog(page)).toContainText(
        "เลือก 7 รายการ · ยอดซื้อรวม ฿1,820.00",
      );
  ...
  await step(
    page,
    "Owner: B3 สต๊อกของทั้งหมด — คลัง Owner ตรงทุกรายการ · ประวัติการซื้อ ฿1,820",
    async () => {
      for (let i = 0; i < MATERIALS.length; i += 1)
        await expect(
          await ownerStock(page, MATERIALS[i], "คลัง Owner"),
        ).toHaveText(`${100 + i * 10}.00`);
      await expect(
        page.getByRole("heading", {
          name: "ประวัติการซื้อและบัญชี · ต้นทุนซื้อเข้าที่แสดง ฿1,820.00",
        }),
      ).toBeVisible();
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      await expect(await ownerStock(page, BOX, "คลัง Owner")).toHaveText(
        "100.00",
      );
    },
  );
  ...
  await step(
    page,
    "Owner: B4 คลัง Owner ลด (กล่อง 40 · กระดาษรอง 80) · สาขายังเป็น 0 จนกว่าจะยืนยันรับ",
    async () => {
      await expect(await ownerStock(page, BOX, "คลัง Owner")).toHaveText(
        "40.00",
      );
      await expect(await ownerStock(page, PAPER, "คลัง Owner")).toHaveText(
        "80.00",
      );
  ...
  await step(
    ...
    async () => {
      ...
      await expect(page.getByRole("dialog")).toHaveCount(0);
      await expect(
        await ownerStock(page, "น้ำพริกหลอด", "คลัง Owner"),
      ).toHaveText("100.00");
    },
  );
  ...
  await step(
    ...
    async () => {
      ...
      await submitAndExpectError(
        page,
        "กรอกจำนวนน้ำพริกที่จัดสรรเป็นตัวเลขมากกว่าศูนย์",
      );
    },
  );
  ...
  await step(
    page,
    "Owner: B5 จัดสรรน้ำพริก 40 หลอดไปศาลาแดง → คลัง Owner 60 · ศาลาแดง 40",
    async () => {
      ...
      await expect(await ownerStock(page, "น้ำพริกหลอด", "ศาลาแดง")).toHaveText(
        "40.00",
      );
      await expect(await ownerStock(page, "น้ำพริกหลอด", "มีนบุรี")).toHaveText(
        "0.00",
      );
    },
  );
  ...
  await step(
    page,
    "สาขาศาลาแดง: B4 ยืนยันรับกล่อง 55 จากที่ส่ง 60 โดยไม่กรอกเหตุผล → กรอกเหตุผลส่วนต่าง",
    async () => {
      await signInAs(page, ACCOUNTS.saladaeng);
      await tab(page, "กรอกรายวัน");
      const pending = tableSection(
        page,
        "รายการวัสดุรอยืนยันรับ (Pending material receipts)",
      );
      await expect(
      ...
      await expect(
        pending.getByRole("row").filter({ hasText: PAPER }),
      ).toHaveCount(0);
  ...
  await step(
    page,
    "สาขาศาลาแดง: B4 รับ 61 > ส่ง 60 → จำนวนรับจริงเกินจำนวนที่ส่ง",
    async () => {
      const pending = tableSection(
        page,
        "รายการวัสดุรอยืนยันรับ (Pending material receipts)",
  ...
  await step(
    page,
    "สาขาศาลาแดง: B4 รับ 55 + เหตุผล → ยืนยันแล้ว · รายการหายจากรอยืนยัน (ยืนยันซ้ำไม่ได้)",
    async () => {
      const pending = tableSection(
        page,
        "รายการวัสดุรอยืนยันรับ (Pending material receipts)",
      ...
      await pending.getByLabel(`จำนวนที่รับจริง ${BOX}`).fill("55");
      await pending
        .getByLabel(`เหตุผลส่วนต่าง ${BOX}`)
        .fill("กล่องเสียหาย 5 ใบ");
      await pointAndClick(
        page,
        pending.getByRole("button", { name: "ยืนยันรับ" }),
      );
      await expect(
        page.getByRole("main").getByText(`ยืนยันรับ ${BOX} แล้ว`),
      ).toBeVisible();
  ...
  await step(
    page,
    "สาขาศาลาแดง: B4/B5 สต๊อกวัสดุ กล่อง 55 ชิ้น · น้ำพริกตั้งต้นจาก Owner 40",
    async () => {
      await expect(
  ...
  await step(
    page,
    "สาขามีนบุรี: B4 ยืนยันรับกระดาษรองครบ 30 (ไม่ต้องกรอกเหตุผล)",
    async () => {
      ...
      await expect(
        page.getByRole("main").getByText(`ยืนยันรับ ${PAPER} แล้ว`),
      ).toBeVisible();
      await tab(page, "สต๊อก");
      await expect(
        cellOf(
          tableSection(page, "สต๊อกวัสดุ 7 รายการ (Material inventory)"),
          [PAPER],
          2,
        ),
      ).toHaveText("30 ชิ้น");
  ...
  await step(
    page,
    "Owner: B4 สต๊อกของทั้งหมด — ศาลาแดง กล่อง 55 · มีนบุรี กระดาษรอง 30 · รายงาน audit trail",
    async () => {
      await signInAs(page, ACCOUNTS.owner);
      await tab(page, "สต๊อกของทั้งหมด");
      await expect(await ownerStock(page, BOX, "ศาลาแดง")).toHaveText("55.00");
      await expect(await ownerStock(page, PAPER, "มีนบุรี")).toHaveText(
        "30.00",
      );
      await expect(await ownerStock(page, BOX, "คลัง Owner")).toHaveText(
        "40.00",
  ...
  await step(
    page,
    "Owner: B6 ค่าใช้จ่าย Owner จำนวนเงิน 0 → บล็อก · รายละเอียดว่าง → กรอกรายละเอียด",
    async () => {
      await tab(page, "รายงาน");
      await button(page, "ค่าใช้จ่าย Owner");
      await page
        .getByLabel(/หมวดค่าใช้จ่าย/)
        .selectOption({ label: "ค่าเช่า" });
  ...
  await step(
    page,
    "Owner: B6 ค่าเช่า ฿5,000 → รายงาน: ตารางค่าใช้จ่าย Owner + สรุปผลรวม",
    async () => {
      ...
      await expect(cellOf(expenses, ["ค่าเช่าร้าน B6"], 1)).toHaveText(
        "ค่าเช่า",
      );
      await expect(cellOf(expenses, ["ค่าเช่าร้าน B6"], 3)).toHaveText(
        "Owner เลนบี",
      );
      await expect(cellOf(expenses, ["ค่าเช่าร้าน B6"], 4)).toHaveText(
        "5,000.00",
      );
      const summary = tableSection(page, "สรุปผลรวม");
      await expect(cellOf(summary, ["↳ ค่าใช้จ่าย Owner"], 1)).toHaveText(
        "5,000.00",
      );
      // 1,820 materials + 2,000 chili (100 × ฿20) + 5,000 expense
      await expect(cellOf(summary, ["↳ ซื้อวัสดุบรรจุภัณฑ์"], 1)).toHaveText(
        "1,820.00",
      );
      await expect(cellOf(summary, ["↳ ซื้อวัตถุดิบ / ETC"], 1)).toHaveText(
        "2,000.00",
      );
      await expect(cellOf(summary, ["ต้นทุนรวมทั้งหมด"], 1)).toHaveText(
        "8,820.00",
      );
      await expect(
        cellOf(
          tableSection(page, "ประวัติจัดสรรน้ำพริกโดย Owner"),
          ["ศาลาแดง"],
          2,
        ),
  ...
  await step(page, "Owner: B6 Log มีรายการค่าใช้จ่าย Owner ฿5000", async () => {
    await tab(page, "Log");
    await expect(
      logEntry(page, "ค่าใช้จ่าย Owner", "ค่าเช่าร้าน B6"),
    ).toHaveCount(1);
  });

  await step(
    page,
    "Owner: B8 รายการที่ยกเลิกไม่ได้ (บันทึกการตั้งค่า) ไม่มีปุ่มยกเลิก",
    async () => {
      const config = page
        .getByRole("main")
        .locator("details")
        .filter({
          has: page.locator("summary").filter({ hasText: "บันทึกการตั้งค่า" }),
        })
        .first();
      await pointAndClick(page, config.locator("summary"));
      await expect(
        config.getByRole("button", { name: "แก้รายการผิดด้วยการยกเลิก" }),
      ).toHaveCount(0);
    },
  );
  ...
  await step(
    page,
    "Owner: B8 ยกเลิกการซื้อสติกเกอร์โลโก้ โดยไม่กรอกเหตุผล → กรอกเหตุผลยกเลิกรายการ",
    async () => {
  ...
  await step(
    page,
    "Owner: B8 กรอกเหตุผลแล้วยืนยัน → ยกเลิกสำเร็จ · Log มีรายการยกเลิกรายการ",
    async () => {
      const entry = logEntry(page, "บันทึกซื้อวัสดุเข้าคลัง Owner", STICKER);
      ...
      await expect(
        logEntry(page, "ยกเลิกรายการ", "บันทึกซ้ำ B8").filter({
          hasText: "materialReceive",
        }),
      ).toHaveCount(1);
    },
  );
  ...
  await step(
    page,
    "Owner: B8 รายการที่ยกเลิกแล้วมีเครื่องหมาย ยกเลิกแล้ว และไม่มีปุ่มยกเลิกซ้ำ",
    async () => {
      const entry = logEntry(page, "บันทึกซื้อวัสดุเข้าคลัง Owner", STICKER);
      await expect(entry).toContainText("ยกเลิกแล้ว");
      await expect(
        entry.getByRole("button", {
          name: /ยืนยันยกเลิก|แก้รายการผิดด้วยการยกเลิก/,
        }),
      ).toHaveCount(0);
      await expect(logEntry(page, "ยกเลิกรายการ", "บันทึกซ้ำ B8")).toHaveCount(
        1,
      );
    },
  );
  ...
  await step(
    page,
    "Owner: B8 สต๊อกถอยกลับ — สติกเกอร์โลโก้ คลัง Owner 150 → 0 · ต้นทุนซื้อวัสดุ 1,820 → 1,520",
    async () => {
      await tab(page, "สต๊อกของทั้งหมด");
      await expect(await ownerStock(page, STICKER, "คลัง Owner")).toHaveText(
        "0.00",
      );
      await expect(
        page.getByRole("heading", {
          name: /ประวัติการซื้อและบัญชี · ต้นทุนซื้อเข้าที่แสดง ฿3,520\.00/,
        }),
      ).toBeVisible();
      await tab(page, "รายงาน");
      await expect(
        cellOf(tableSection(page, "สรุปผลรวม"), ["↳ ซื้อวัสดุบรรจุภัณฑ์"], 1),
      ).toHaveText("1,520.00");
    },
  );
  ...
  await step(
    page,
    "ระบบ: B8 reload → การยกเลิกยังอยู่ (สติกเกอร์โลโก้ 0)",
    async () => {
      await page.reload();
      await tab(page, "สต๊อกของทั้งหมด");
      await expect(await ownerStock(page, STICKER, "คลัง Owner")).toHaveText(
...
test("Lane B: B7 หลังโหลดข้อมูลจำลอง Owner ปลดล็อกวันศาลาแดง → สาขาบันทึกได้อีก · ปลดล็อกซ้ำ → วันนี้ยังไม่ได้ปิด", async ({
  ...
  await step(
    page,
    "สาขาศาลาแดง: B7 วันนี้ปิดแล้วใน sample → ปุ่มกรอกข้อมูลล็อก",
    async () => {
      await signInAs(page, ACCOUNTS.saladaeng);
      await tab(page, "กรอกรายวัน");
  ...
  await step(
    page,
    "Owner: B7 ปลดล็อกวันศาลาแดง ไม่กรอกเหตุผล → กรอกเหตุผลปลดล็อก · กรอกแล้วผ่าน",
    async () => {
      await signInAs(page, ACCOUNTS.owner);
      await tab(page, "รายงาน");
  ...
  await step(
    page,
    "Owner: B7 ปลดล็อกศาลาแดงซ้ำ (เปิดอยู่แล้ว) → วันนี้ยังไม่ได้ปิด",
    async () => {
      await button(page, "ปลดล็อกวัน");
      await page
        .getByLabel(/สาขาที่ปลดล็อก/)
  ...
  await step(
    page,
    "สาขาศาลาแดง: B7 หลังปลดล็อก ซื้อข้าวเหนียวดิบ 5 กก. บันทึกได้",
    async () => {
      await signInAs(page, ACCOUNTS.saladaeng);
      await tab(page, "กรอกรายวัน");
      ...
      await expect(
        logEntry(page, "ซื้อข้าวเหนียวเข้าสต๊อก", "ร้านข้าว B7"),
      ).toHaveCount(1);
    },
  );
  ...
  await step(
    page,
    "สาขามีนบุรี: B7 ยังปิดอยู่ (ปลดล็อกเฉพาะศาลาแดง)",
    async () => {
      await signInAs(page, ACCOUNTS.minburi);
      await tab(page, "กรอกรายวัน");
      await expect(page.getByRole("main")).toContainText("ปิดวันแล้ว");
...
test("Lane B: B9 กระดิ่งแจ้งเตือน — seed ว่างมีเฉพาะตั้งค่าวัสดุ · หลัง sample คลิกแล้วไป tab ที่เกี่ยวข้อง", async ({
  ...
  await step(
    page,
    "Owner: B9 seed ว่าง → แจ้งเตือน 1 รายการ (ตั้งค่าวัสดุยังไม่ครบ 7) ไม่มี alert ของ Lot",
    async () => {
      await signInAs(page, ACCOUNTS.owner);
      ...
      await expect(items.first()).toContainText(
        "ตั้งค่าวัสดุยังไม่ครบ 7 รายการ",
      );
  ...
  await step(
    page,
    "Owner: B9 คลิกแจ้งเตือน → ไปหน้าตั้งค่า popover ปิด",
    async () => {
      await pointAndClick(
        page,
        popover.getByRole("button", { name: /ตั้งค่าวัสดุยังไม่ครบ/ }),
      );
  ...
  await step(
    page,
    "Owner: B9 ตั้งฐานวัสดุครบ → กระดิ่งเหลือ 0 · popover ไม่มีงานค้าง",
    async () => {
      await setMaterialPars(
        page,
        () => "50",
      ...
      await pointAndClick(
        page,
        popover.getByRole("button", { name: "ปิด", exact: true }),
      );
    },
  );
  ...
  await step(
    page,
    "ระบบ: B9 โหลดข้อมูลจำลอง 7 วัน แล้วเข้าสู่ระบบ Owner ใหม่",
    async () => {
      await loadSampleData(page);
      await signInAs(page, ACCOUNTS.saladaeng);
      await signInAs(page, ACCOUNTS.owner);
  ...
  await step(
    page,
    "Owner: B9 sample (ทุก Lot จบ วัสดุตั้งครบ) → กระดิ่ง 0 ไม่มี alert ผิด ๆ",
    async () => {
      await expect(bell).toHaveAccessibleName("การแจ้งเตือน 0 รายการ");
      await pointAndClick(page, bell);
      await expect(popover).toContainText("ไม่มีงานค้าง");
  ...
  await step(
    page,
    "Owner: B9 สร้าง PO เนื้อใหม่ → กระดิ่ง 1 (รอ Foodiva ออก Invoice)",
    async () => {
  ...
  await step(
    page,
    "Owner: B9 เปิด popover คลิกแจ้งเตือน → ไปหน้าใบสั่งซื้อ PO ที่มี Lot นั้น",
    async () => {
      await pointAndClick(page, bell);
      const item = popover.getByRole("button", {
        name: /รอ Foodiva ออก Invoice · F\d{6}-\d{3}/,
      });
...
test("E2E-B1: ข้อความ error ตั้งค่าตัวเลขต้องใช้ชื่อช่องภาษาไทย ไม่ใช่ key (boxPrice)", async ({
  ...
  await step(
    page,
    "Owner: ราคากล่อง -5 → ข้อความต้องระบุราคากล่องมาตรฐาน",
    async () => {
      await signInAs(page, ACCOUNTS.owner);
      await tab(page, "ตั้งค่า");
      const pricing = tableSection(page, S_PRICING);
...
test("E2E-B2: ส่งวัสดุจำนวนทศนิยม ต้องขึ้นข้อความในฟอร์ม ไม่ใช่ native bubble", async ({
  ...
  await step(
    page,
    "Owner: ส่งกล่อง 2.5 ชิ้นไปศาลาแดง → ข้อความ inline",
    async () => {
      await signInAs(page, ACCOUNTS.owner);
      await tab(page, "สต๊อกของทั้งหมด");
      await button(page, "ส่งวัสดุไปสาขา");
...
test("E2E-B3: รายการที่ถูกยกเลิกใน Log ต้องมีเครื่องหมายยกเลิก และไม่มีปุ่มยกเลิกซ้ำ", async ({
  ...
  await step(
    page,
    "ระบบ: reload → รายการที่ยกเลิกต้องมีเครื่องหมาย และไม่มีปุ่มยกเลิก",
    async () => {
      await page.reload();
      await tab(page, "Log");
      const entry = logEntry(page, "บันทึกซื้อวัสดุเข้าคลัง Owner", BOX);
