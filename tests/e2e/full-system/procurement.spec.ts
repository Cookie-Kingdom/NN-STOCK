...
const registerRow = (page: Page) =>
  tableSection(page, TRACE)
    ...
    .filter({
      has: page
        .getByRole("button", { name: `ขยายรายละเอียด ${LOT}` })
        .or(page.getByRole("button", { name: `ย่อรายละเอียด ${LOT}` })),
    });
...
test("C1–C13 จัดซื้อ → รมควัน → ขนส่งขาไป: PO 500 → Invoice 490/10 → PO รมควัน 490 = 107,800 → ส่ง 490 → Chef House รับ 488", async ({
  ...
  await step(
    page,
    "Owner: C1 PO 500 กก. @250 หมายเหตุ QA-C1 → preview สด → บันทึก → ตารางแสดงเลข PO / Lot",
    async () => {
      await field(page, /ราคาเนื้อ/, "250");
  ...
  await step(
    page,
    'Owner: C5 ใบขนส่งก่อน Foodiva ยืนยัน → ป้าย "รอ Foodiva ออก Invoice" ไม่มีปุ่มทำใบขนส่ง (พฤติกรรมจริง: ปุ่มไม่มี ไม่ใช่ข้อความ store)',
    async () => {
      await tab(page, "ใบขนส่ง");
      const row = rowIn(page, "รายการขนส่งตาม Lot", LOT);
      await expect(row).toContainText("รอ Foodiva ออก Invoice");
      await expect(row).toContainText(
        "พร้อมส่งเชียงใหม่ 0.00 กก. · รอทำใบขนส่ง",
      );
      await expect(
        page.getByRole("button", { name: "ทำใบขนส่งขาไป" }),
      ).toHaveCount(0);
    },
  );
  ...
  await step(
    page,
    "Owner: C3 เอกสารและ Traceability (อ่านอย่างเดียว): PO อยู่ในทะเบียน รอ Invoice Foodiva · ไม่มีปุ่มบันทึกเอกสาร",
    async () => {
      ...
      await expect(traceDetail(page, "Invoice Foodiva")).toContainText(
        "รอ Foodiva",
      );
      await pointAndClick(
        page,
  ...
  await step(
    page,
    "Foodiva: C4 Invoice 490 ส่ง + 10 รอ Owner → บันทึก · count pill หาย · ตารางแสดง 490 / 10 / คงเหลือ 500",
    async () => {
  ...
  await step(
    page,
    'Owner: C5 หลัง Foodiva ยืนยันแต่ยังไม่มี PO รมควัน → ใบขนส่ง "รอ Owner ออก PO รมควัน" · หน้า PO มีปุ่ม "ไปใบสั่ง PO โรงรมควัน"',
    async () => {
      await tab(page, "ใบขนส่ง");
      const row = rowIn(page, "รายการขนส่งตาม Lot", LOT);
      await expect(row).toContainText(
        "พร้อมส่งเชียงใหม่ 490.00 กก. · รอทำใบขนส่ง",
      );
  ...
  await step(
    page,
    "Owner: C6 ออก PO รมควัน 491 > 490 พร้อมส่ง → บล็อก",
    async () => {
      await button(page, "ออก PO รมควันเนื้อ");
      await expect(
        dialog(page).getByLabel(/โรงรม \/ ผู้ให้บริการ/),
      ).toHaveValue("Chef House");
      await expect(dialog(page).getByLabel(/Raw Meat Quantity/)).toHaveValue(
        "490",
      );
      await field(page, /Raw Meat Quantity/, "491");
      await submitAndExpectError(
        page,
        "น้ำหนักใน PO รมควันเกินยอดที่ Foodiva ระบุว่าพร้อมส่งเชียงใหม่",
      );
    },
  );
  await step(
    page,
    "Owner: C6 PO รมควัน 490 → บันทึก · อัตรา ฿220 / กก. · รอ Chef House ยืนยัน 1 ใบ",
    async () => {
      await field(page, /Raw Meat Quantity/, "490");
      await saveEntry(page);
      const row = rowIn(page, "รายการ PO โรงรมควัน", LOT);
      await expect(row).toContainText(
        "FD-INV-C4 · พร้อมส่งเชียงใหม่ 490.00 กก.",
      );
  ...
  await step(
    page,
    'Owner: C9 gate ก่อน Chef House ยืนยัน PO → ใบขนส่งแสดง "รอ Chef House รับ PO"',
    async () => {
      await tab(page, "ใบขนส่ง");
      await expect(rowIn(page, "รายการขนส่งตาม Lot", LOT)).toContainText(
  ...
  await step(
    page,
    "Chef House: C6 ยืนยันรับ PO: ชื่อผู้รับว่าง → กรอกชื่อผู้รับ PO · กรอกแล้ว → ปุ่มยืนยันหาย (ยืนยันซ้ำไม่ได้จาก UI)",
    async () => {
      await button(page, "ยืนยันรับ PO รมควัน");
  ...
  await step(
    ...
    async () => {
      ...
      await expect(page.getByRole("button", { name: "ชำระเงิน" })).toHaveCount(
        0,
      );
    },
  );
  ...
  await step(
    page,
    'Owner: C9 gate ก่อนชำระ → ใบขนส่งแสดงปุ่ม "ชำระ Invoice เพื่อเรียกรถ" ไม่มีทำใบขนส่ง',
    async () => {
      await tab(page, "ใบขนส่ง");
      await expect(
        rowIn(page, "รายการขนส่งตาม Lot", LOT).getByRole("button", {
          name: "ชำระ Invoice เพื่อเรียกรถ",
        }),
      ).toBeVisible();
      await expect(
        page.getByRole("button", { name: "ทำใบขนส่งขาไป" }),
  ...
  await step(
    ...
    async () => {
      ...
      await expect(page.getByRole("button", { name: "ตรวจยอด" })).toHaveCount(
        0,
      );
    },
  );
  ...
  await step(
    page,
    "Owner: C9 ทำใบขนส่งขาไป: ต้นทาง=ปลายทาง → ต้นทางและปลายทางต้องต่างกัน",
    async () => {
      await tab(page, "ใบขนส่ง");
      await button(page, "ทำใบขนส่งขาไป");
      ...
      await expect(form.getByLabel(/น้ำหนักที่ส่งเที่ยวนี้/)).toHaveValue(
        "490",
      );
      await field(page, /เวลารถรับ/, "06:30");
      await field(page, /ประเภทรถ/, "รถห้องเย็น");
  ...
  await step(
    page,
    "Owner: C9 ส่ง 600 > 490 พร้อมส่ง → น้ำหนักใบขนส่งเกินยอดที่ Foodiva ระบุว่าพร้อมส่งเชียงใหม่",
    async () => {
      await dialog(page)
        .getByLabel(/ต้นทาง/)
  ...
  await step(
    page,
    "Owner: C9 ส่ง 490 หมายเหตุ QA-C9 → ใบขนส่งแสดง 490 กก. · กท 1001 · พรีวิว / PDF · stage รับที่ Chef House",
    async () => {
  ...
  await step(
    page,
    "Owner: C1 หมายเหตุ PO ยังเป็น QA-C1 หลังทำใบขนส่ง (BUG-4)",
    async () => {
      await expectPrintPopup(page, "พิมพ์ / PDF", async (body) => {
        await expect(body).toContainText("QA-C1");
  ...
  await step(
    page,
    "Chef House: C10 ยืนยันรับเนื้อ pill 1 · Lot รอรับ 490 กก. รถห้องเย็น · กท 1001",
    async () => {
      await signInAs(page, ACCOUNTS.chef);
  ...
  await step(
    page,
    "Chef House: C10 น้ำหนักรับจริง 0 → กรอกน้ำหนักรับเป็นตัวเลขมากกว่าศูนย์",
    async () => {
      await pointAndClick(
        page,
        rowIn(page, "Lot ที่รอยืนยันรับ", LOT).getByRole("button", {
          name: "ยืนยันรับเนื้อ",
        }),
      );
      await expect(
  ...
  await step(
    ...
    async () => {
      ...
      await saveEntry(page);
      await expect(
        page.getByText("ไม่มี Lot รอยืนยันรับในขณะนี้"),
      ).toBeVisible();
  ...
  await step(
    page,
    "Owner: C13 เอกสารและ Traceability: Invoice Foodiva 500 · PO รมควัน 490 · Invoice Chef House ชำระแล้ว · ใบขนส่ง TR 490 · รับที่ Chef House 488",
    async () => {
      await tab(page, "เอกสารและ Traceability");
      const row = registerRow(page);
      await expect(row).toContainText("ก่อนสโมค");
  ...
  await step(
    page,
    "Foodiva: C13 PO และสต๊อก: คงเหลือ Foodiva 0 · สถานะส่งให้ Chef House แล้ว",
    async () => {
      await signInAs(page, ACCOUNTS.foodiva);
      const row = rowIn(page, "PO เนื้อที่ต้องออก Invoice", LOT);
...
test("E2E-C2: หน้า Foodiva แสดงเนื้อรอ Owner รับ = 0 หลัง Owner รับครบ 10 กก. (C11)", async ({
  ...
  await step(
    ...
    async () => {
      await signInAs(page, ACCOUNTS.foodiva);
      await expect(
        rowIn(page, "PO เนื้อที่ต้องออก Invoice", LOT).getByRole("cell").nth(5),
      ).toHaveText("0.00 กก.");
      await expect(
        stat(page, "เนื้อส่วนที่เหลือรอ Owner รับ (Waste)"),
...
test("E2E-C3: Chef House รับเนื้อ 300 จากที่ส่ง 500 (เกิน tolerance 20 %) ต้องกรอกเหตุผลส่วนต่าง (C10)", async ({
  ...
  await step(
    page,
    "Chef House: C10 รับ 300 จาก 500 → กรอกเหตุผลส่วนต่าง",
    async () => {
      await signInAs(page, ACCOUNTS.chef);
      await tab(page, "ยืนยันรับเนื้อ");
      await pointAndClick(
...
test('E2E-C4: Chef House "ดู PO รมควัน" แสดงเลข PO รมควันที่บันทึกแล้ว ไม่ใช่เลขฉบับร่างถัดไป (C6)', async ({
  ...
  await step(
    page,
    "ระบบ: Owner PO 500 → Foodiva Invoice 500 → Owner PO รมควัน 500",
    async () => {
      await startFresh(page);
      await signInAs(page, ACCOUNTS.owner);
      await ownerCreatesMeatPo(page, "500");
