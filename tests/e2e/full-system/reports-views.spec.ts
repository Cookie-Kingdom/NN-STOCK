...
test.skip(
  ...
  "sample data and local sign-in only exist in pnpm test:e2e:local",
);

const bangkok = (offsetDays = 0) => {
  const now = new Date(
...
/** sevenDayRoleplay(today): the first sample day is today − 6. */
const TODAY = bangkok();
...
/** "label … value" as rendered text (cards put the value in a sibling element). */
const pair = (label: string, value: string) =>
  new RegExp(`${esc(label)}\\s*${esc(value)}`);

async function expectCleanNumbers(page: Page) {
  await expect(main(page)).not.toContainText(/NaN|undefined|Infinity/);
}

async function withSample(page: Page, account: AccountKey) {
...
test.describe("Lane F · รายงาน เอกสาร มุมมองอ่าน", () => {
  test("F1 Owner แดชบอร์ด: KPI กราฟ และรายการแจ้งเตือนจากข้อมูล 7 วัน", async ({
    page,
  }) => {
  ...
  test("F2 สต๊อกของทั้งหมด: เนื้อ ข้าว น้ำพริก วัสดุ ตรงกับหน้าสต๊อกของ Chef House และสาขา", async ({
    ...
    await step(page, "สาขาศาลาแดง: หน้าสต๊อกตรงกับของ Owner", async () => {
      ...
      await tab(page, "สต๊อก");
      await expect(rowIn(page, "สต๊อกเนื้อ · ศาลาแดง", LOT)).toContainText(
  ...
  test("F5 รายงาน: ยอดขาย ต้นทุน ส่วนต่าง ตามช่วงวันที่และสาขา รวม ETC", async ({
    ...
    const summary = "สรุปผลรวม";
    const filters = (p: Page) =>
      main(p)
  ...
  test("F8 Empty state: ข้อมูลตั้งต้นว่าง เปิดทุกเมนูของทุกบัญชี ไม่มี error และไม่มี NaN", async ({
    ...
    page.on("console", (message) => {
      // Playwright's screenshot hides the caret with an injected style, which React
