...
test("ข้อ 1 + 4.1 + 4 + 2: Owner/Chef House เห็นตาราง Packing List 3 ชั้น · Chef กรอกได้แค่ช่องเหลือง · กล่องรมควันต้องกรอกกิโล · ผลผลิตรวมเป็นยอดตั้งต้นจัดสรรเป็นกิโล · ไม่มีคำว่าถุง · สลิปค่ารมดาวน์โหลดได้", async ({
  ...
  await step(
    page,
    "Owner → Foodiva → Owner: PO 100 · Invoice · Request 60 · ใบขนส่ง + Packing List 30 + 30 (Inv. Weight 65, Lost 60)",
    async () => {
      await signInAs(page, ACCOUNTS.owner);
  ...
  await step(
    ...
    async () => {
      ...
      await expect(list).toContainText(/Sliced Weight Lost\s*60\.00 กก\./);
  ...
  await step(
    ...
    async () => {
      ...
      await expect(dialog).toContainText(/Sliced Weight Lost\s*60\.00 กก\./);
      await expect(
