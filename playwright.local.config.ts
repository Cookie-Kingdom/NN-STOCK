...
export default defineConfig({
  ...
  workers: 1,
  outputDir: `artifacts/playwright${lane}`,
  reporter: [
    ["list"],
    ["html", { outputFolder: `artifacts/playwright-report${lane}`, open: "never" }],
    // Read by scripts/e2e-flow-report.mjs to render one flow page per spec.
    ["json", { outputFile: `artifacts/playwright-results${lane}.json` }],
  ],
