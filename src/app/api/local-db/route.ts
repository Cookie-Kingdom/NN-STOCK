...
let local: Promise<{ db: import("node:sqlite").DatabaseSync } & typeof import("@/lib/local-db.server")> | undefined;
const open = () => (local ??= import("@/lib/local-db.server").then((mod) => ({ ...mod, db: mod.openLocalDb(process.env.LOCAL_DB_FILE || ".local/app.db") })));
...
export async function POST(request: Request) {
  ...
  const account = accountById((await cookies()).get(LOCAL_ACCOUNT_COOKIE)?.value);
  const { payload, expectedRevision } = await request.json();
  try {
    return Response.json(saveState(db, account, payload, expectedRevision));
  } catch (error) {
    return Response.json({ message: (error as Error).message }, { status: 409 });
  }
}
