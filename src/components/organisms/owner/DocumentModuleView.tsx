...
export function DocumentModuleView({ db, open }: { db: Database; open: (kind: string, lotId?: string) => void }) {
  ...
  const kpis = [
    ...
    { label: "Supplier Invoice ค้างชำระ", value: outstandingSupplier.length, caption: "ใบ" },
    { label: "Smoking Invoice ค้างชำระ", value: outstandingSmoking.length, caption: "ใบ" },
    { label: "Average Yield", value: `${fmt(averageYield(db))}%`, caption: "ทุก Smoke Batch" },
  ];
  return <div className="grid gap-6">
    <PanelHeading
      ...
      aside={<Button variant="secondary" onClick={printDocument}>พิมพ์ / บันทึก PDF</Button>}
    />
    <section className="grid grid-cols-4 overflow-hidden rounded-lg border border-border bg-surface max-[1050px]:grid-cols-2 max-[640px]:grid-cols-1">
      {kpis.map((kpi) => (
        <article key={kpi.label} className="grid gap-2 px-6 py-5.5">
          <small className="text-body-sm text-text-secondary">{kpi.label}</small>
          <strong className="text-num-lg text-text-primary tabular-nums">{kpi.value}</strong>
          <small className="text-body-sm text-text-secondary">{kpi.caption}</small>
        </article>
      ))}
    </section>
    <DataTable title="Beef Lot traceability และสถานะสต๊อก" columns={["PO / Beef Lot", "ซื้อจาก", "Foodiva", "ที่โรงรม", "Steak", "หลังรม", "Loss / Yield", "เอกสารต่อไป"]} rows={db.lots.map((lot) => {
      ...
      return [
        ...
        <ButtonRow key="action">
          <Button variant="table" onClick={() => open("taxDocument", lot.id)}>ภาษี</Button>
          {entries(db, "foodDivaConfirm", lot.id).length > 0 && !hasSmokeOrder && <Muted as="span">ออก PO จากเมนูใบสั่ง PO โรงรมควัน</Muted>}
          {rawAtFoodiva(db, lot) > 0.001 && <Button variant="table" onClick={() => open("steakTransfer", lot.id)}>โอนไป Steak</Button>}
          {entries(db, "smokeOrder", lot.id).length > 0 && <Muted as="span">Chef_house ออกใบวางบิลจากเมนูงานผลิต</Muted>}
        </ButtonRow>,
      ];
    })} />
    ...
    <DataTable title="Supplier Invoice และ Tax documents" columns={["ประเภท", "เลขที่", "วันที่", "PO / Lot", "ยอดรวม", "VAT", "สถานะ", "เอกสาร"]} rows={[
      ...
      ...entries(db, "smokingInvoice").map((entry) => { const lot = db.lots.find((item) => item.id === entry.lotId); const status = smokingInvoiceStatus(db, entry); return ["Smoking Service Invoice", entry.values.invoiceNumber, entry.values.invoiceDate, `${lot?.poId || "-"} / ${entry.lotId}`, `฿${fmt(n(entry.values, "netPayable"))}`, `฿${fmt(n(entry.values, "vat"))}`, status, <ButtonRow key={entry.id}><DocumentPrintButton title="Smoking Service Invoice" number={entry.values.invoiceNumber} rows={[["วันที่ Invoice", entry.values.invoiceDate], ["ผู้ให้บริการ", entry.values.serviceProvider], ["PO", lot?.poId || "-"], ["Beef Lot", entry.lotId], ["จำนวนคิดค่าบริการ", `${fmt(n(entry.values, "serviceQuantity"))} กก.`], ["ยอดก่อน VAT", `฿${fmt(n(entry.values, "amountBeforeVat"))}`], ["VAT", `฿${fmt(n(entry.values, "vat"))}`], ["หัก ณ ที่จ่าย", `฿${fmt(n(entry.values, "withholdingTax"))}`], ["ยอดสุทธิ", `฿${fmt(n(entry.values, "netPayable"))}`], ["ไฟล์แนบ", entry.values.attachment]]} />{status === "รอตรวจยอด" && <Button variant="table" onClick={() => open("invoiceReview", entry.lotId)}>ตรวจยอด</Button>}{status === "รอชำระ" && <Button variant="table" onClick={() => open("invoicePayment", entry.lotId)}>ชำระเงิน</Button>}</ButtonRow>]; }),
    ]} />
    ...
    <Footnote className="-mt-1">ข้อมูลบันทึกด้วยบทบาทและเวลาอัตโนมัติใน Log ของระบบ เอกสารที่ยืนยันแล้วใช้การยกเลิก/ปรับปรุงแทนการลบ เพื่อให้ย้อนรอยได้</Footnote>
  </div>;
}
