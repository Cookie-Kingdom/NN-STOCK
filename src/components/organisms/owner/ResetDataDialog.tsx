...
export function ResetDataDialog({
  ...
  const replace = (
    build: () => ReturnType<typeof sevenDayRoleplay>,
    message: string,
  ) => {
    try {
      saveDatabase(build());
      ...
      onDone(
        error instanceof Error
          ? error.message
          : "บันทึกไม่ได้ กรุณาตรวจพื้นที่จัดเก็บเบราว์เซอร์",
      );
    }
  };
    ...
    <Dialog title="รีเซ็ตข้อมูลระบบ?" className="w-112.5" onClose={onClose}>
      <DialogBody>
        <p className="mt-0 mb-5">
          ลบข้อมูลชุดปัจจุบันในเบราว์เซอร์นี้ ควรส่งออกก่อนหากต้องการเก็บไว้
        </p>
        <ButtonRow>
          ...
          <Button
            ...
            onClick={() =>
              replace(
                () => sevenDayRoleplay(date),
                "โหลดข้อมูลตัวอย่างครบ 7 วันแล้ว",
              )
            }
          >
          ...
          <Button
            ...
            onClick={() =>
              replace(
                () => thirtyDayRoleplay(date),
                "โหลดข้อมูลตัวอย่างครบ 30 วันแล้ว",
              )
            }
          >
          ...
          <Button
            variant="primary"
            onClick={() =>
              replace(() => structuredClone(seed), "เริ่มชุดข้อมูลใหม่แล้ว")
            }
          >
            ยืนยันรีเซ็ต
          </Button>
