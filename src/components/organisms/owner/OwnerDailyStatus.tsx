...
import { requiredDailyKinds, sevenDayRangeStart } from "@/components/organisms/owner/ownerDaily";
...
export function OwnerDailyStatus({ db, date }: { db: Database; date: string }) {
  ...
  const visibleBranches = branchFilter === "ทั้งหมด" ? branches : [branchFilter];
  const dates: string[] = [];
  for (let cursor = dayNumber(startDate); cursor <= dayNumber(date); cursor += dayMs)
    ...
    dates.flatMap((workDate) => {
      ...
      if (!missing.length)
        return [[
          ...
          "กรอกครบทุกหัวข้อ",
          "0 วัน",
          <Badge tone="success" key="complete">ครบแล้ว</Badge>,
        ]];
      return missing.map((kind) => [
        ...
        <Badge tone="danger" key="pending">ค้างกรอก</Badge>,
      ]);
    }),
  );
  return (
