...
export function MeatMovementLogView({ db }: { db: Database }) {
  ...
  const locationRows = lots.flatMap((lot) => {
    ...
    const foodivaSmoked = Math.max(0, returnReceived - n(lot.values, "centralKg"));
    const chefSmoked = lot.stage === 6 && !entries(db, "return", lot.id).length
      ? produced(db, lot.id)
      : 0;
    return [
      [lot.poId, lot.id, "Foodiva · เนื้อดิบ", `${fmt(rawAtFoodiva(db, lot))} กก.`, "คงเหลือจาก PO ก่อนส่ง Chef_house"],
      [lot.poId, lot.id, "Foodiva · เนื้อส่วนที่เหลือรอ Owner รับ (Waste)", `${fmt(ownerWasteOutstanding(db, lot.id))} กก.`, `Owner รับแล้ว ${fmt(ownerWasteReceived(db, lot.id))} กก.`],
      [lot.poId, lot.id, "Owner · เนื้อส่วนที่รับแล้ว (Waste)", `${fmt(ownerWasteReceived(db, lot.id))} กก.`, "รับจาก Foodiva สำหรับใช้งาน Owner"],
      ...
      [lot.poId, lot.id, "Foodiva · เนื้อรมควัน", `${fmt(foodivaSmoked)} กก.`, foodivaSmoked > 0 ? "รับจาก Chef_house แล้ว รอ Owner รับเข้าคลังกลาง" : "—"],
